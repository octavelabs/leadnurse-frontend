...
const SLIDE_TYPES = [
  ...
  { value: 'CONTENT', label: 'Content', icon: 'Â¶', desc: 'Heading + rich text body' },
  { value: 'BULLET_LIST', label: 'Key Points', icon: 'â€¢', desc: 'List of bullet points' },
  { value: 'IMAGE', label: 'Image', icon: 'ðŸ–¼', desc: 'Image with optional caption' },
...
const THEMES = [
  ...
  { value: 'blue',    label: 'Lead Blue',      bg: 'bg-primary-700',                           text: 'text-white' },
    ...
    <div className="space-y-3">
        ...
        <div className="grid grid-cols-5 gap-1.5">
          {SLIDE_TYPES.map((t) => (
            <button
              ...
              className={`p-2 rounded-lg border-2 text-center transition-colors ${slide.slideType === t.value ? 'border-primary-600 bg-primary-50' : 'border-gray-200 hover:border-gray-300'}`}
              title={t.desc}
            >
        ...
        <div className="flex gap-2 flex-wrap">
          {THEMES.map((t) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium border-2 transition-colors ${t.bg} ${t.text} ${slide.backgroundTheme === t.value ? 'ring-2 ring-offset-1 ring-primary-700' : 'opacity-70 hover:opacity-100'}`}
            >
              {t.label}
            </button>
          ))}
      ...
      <div>
        ...
        <input
          ...
          placeholder={slide.slideType === 'TITLE' ? 'Chapter or topic titleâ€¦' : slide.slideType === 'QUOTE' ? 'e.g. â€” Florence Nightingale' : 'Slide headingâ€¦'}
        />
      </div>

      {/* Content field â€” varies by type */}
      {slide.slideType === 'BULLET_LIST' ? (
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => set('content', e.target.value)}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-700"
            placeholder="Optional supporting text below the titleâ€¦"
          />
        </div>
      ) : (
        <div>
          ...
          <textarea
            ...
            placeholder={slide.slideType === 'QUOTE' ? '"The nurse is the key worker in healthcare deliveryâ€¦"' : 'Slide contentâ€¦'}
          />
        </div>
      )}
    </div>
      ...
      {questions.map((q, qi) => (
        <div key={qi} className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
          ...
          <input
            ...
            placeholder="Explanation shown after submission (optional)â€¦"
          />
        </div>
      ))}
    ...
    <div className="bg-gray-50 border border-gray-200 rounded-xl overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-white border-b border-gray-200">
        ...
        <div className="flex items-center gap-1 text-xs text-gray-400">
          ...
          {chapter.quiz && <span className="text-green-600 font-medium ml-1">Â· Quiz âœ“</span>}
        </div>
            ...
            <div key={slide.id}>
              {editingSlide?.id === slide.id ? (
                <div className="bg-white border border-primary-200 rounded-xl p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold text-primary-700">Editing Slide {idx + 1}</span>
...
export default function LessonManagementPage() {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div>
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-gray-500 mt-1 text-sm">{course?.title} Â· {chapters.length} chapter{chapters.length !== 1 ? 's' : ''} Â· {chapters.reduce((s, c) => s + c.lessons.length, 0)} slides</p>
          </div>
      ...
      <div className="bg-primary-50 border border-primary-100 rounded-xl p-4 text-sm text-blue-800">
        <strong>How it works:</strong> Add chapters to organise your course. Each chapter contains slides (Title, Content, Key Points, Image, Quote). Add a 2â€“5 question quiz at the end of each chapter to test understanding before learners advance.
      </div>
      ...
      {addingChapter ? (
        <div className="bg-white border border-primary-200 rounded-xl p-4 flex gap-3">
          <input
            ...
            placeholder="Chapter title, e.g. Introduction to Medication Safetyâ€¦"
          />
        ...
        <button
          ...
          className="w-full py-3 border-2 border-dashed border-gray-200 rounded-xl text-sm text-gray-400 hover:border-blue-400 hover:text-primary-600 transition-colors font-medium"
        >
          + Add Chapter
        </button>
      )}
